...
def calcular_valoracion_dcf(wacc, crecimiento_ingresos, crecimiento_fcf, ebitda_2023, impuesto_tasa, capex_sobre_ebitda, acciones_en_circulacion, tipo_cambio, horizonte=5):
    ...
    También devuelve la lista de flujos de caja libre proyectados.
    """
...
tipo_cambio = 20.40  # USD/MXN

# Cálculo del precio objetivo con DCF
precio_objetivo, fcfs = calcular_valoracion_dcf(wacc, crecimiento_ingresos, crecimiento_fcf, ebitda_2023,
...
for i, crecimiento_fcf in enumerate(crecimiento_fcf_range):
    for j, wacc_sens in enumerate(wacc_range):
        precios_objetivo[i, j], _ = calcular_valoracion_dcf(wacc_sens, crecimiento_ingresos, crecimiento_fcf, ebitda_2023,
                                                             impuesto_tasa, capex_sobre_ebitda, acciones_en_circulacion, tipo_cambio)
...
# Graficar los flujos de caja libre proyectados
plt.figure(figsize=(8, 5))
plt.plot(range(1, len(fcfs) + 1), fcfs, marker='o', linestyle='-', color='b', label="FCF Proyectado")
plt.xlabel("Año")
plt.ylabel("Flujo de Caja Libre (millones USD)")
plt.title("Proyección de Flujos de Caja Libre de GRUMA")
plt.legend()
plt.grid(True)
plt.show()
